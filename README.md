...
| `IronProw.IronHive` | IronHive provider 어댑터 — OpenAI · Anthropic · GoogleAI (frontier) · GpuStack (LAN) |
...
`IronProw.IronHive`는 네 가지 provider 어댑터를 제공한다:
...
| 갈래 | 진입점 | 책임 |
|---|---|---|
| **A. Frontier / LAN** | `AddIronHiveOpenAI` · `AddIronHiveAnthropic` · `AddIronHiveGoogleAI` · `AddIronHiveGpuStack` | provider 레지스트리, 우선순위 선택, retry, provider-level fallback, 전환 이벤트(`OnTransition`) |
